...
class ClauseDiff(BaseModel):
    clause: str
    contradiction: str = Field(description="Direct statement of what conflicts between buyer and seller")
    severity: str = Field(description="LOW/MEDIUM/HIGH - color coded severity level")
...
SYSTEM_PROMPT = """You are an in-house legal counsel analyzing buyer documents to identify contradictions with seller baseline terms.

Focus on CONTRADICTIONS ONLY:
- Identify direct conflicts between buyer terms and seller baseline
- Ignore missing terms (silence favors seller)
- Rate severity: LOW (minor variance), MEDIUM (moderate risk), HIGH (major liability exposure)
- Only flag actual contradictions, not missing provisions
...
{{
  ...
  "clause_diffs": [{{
    "clause": "Clause name",
    "contradiction": "Direct statement of the conflict between buyer and seller terms",
    ...
    try:
        ...
        for diff in clause_diffs:
            if isinstance(diff, dict):
                diff.setdefault("contradiction", "Conflict detected between buyer and seller terms")
                diff.setdefault("severity", "MEDIUM")
    ...
    with open(xml_path, "w", encoding="utf-8") as f:
        ...
        if result.clause_diffs:
            ...
            for diff in result.clause_diffs:
                ...
                f.write(f'      <contradiction><![CDATA[{diff.contradiction}]]></contradiction>\n')
                f.write('    </clause_diff>\n')
            f.write('  </clause_differences>\n')
...
def process_contract_text(contract_text):
    ...
    try:
        ...
        temp_file = "temp_pasted_text.txt"
        temp_path = os.path.join(app.config['UPLOAD_FOLDER'], temp_file)
        with open(temp_path, 'w', encoding='utf-8') as f:
            f.write(contract_text)
        
        # Run 3-agent analysis workflow
        from agent1_parser import run_agent1
        final_report_path = run_agent1(temp_path)

        # Load the final analysis result
        with open(final_report_path, 'r', encoding='utf-8') as f:
            final_data = json.load(f)
        
        analysis_result = AnalysisResult(**final_data)
...
def process_contract(filepath, filename):
    try:
        ...
        # Run 3-agent analysis workflow directly on file
        from agent1_parser import run_agent1
        final_report_path = run_agent1(filepath)
